...
pub fn ejecutar_diff(path_archivo_1: String, path_archivo_2: String) -> Result<(), String> {
    ...
    print_diff(&grilla_con_resultado_lcs, &lineas_f1, &lineas_f2, lineas_f1.len(), lineas_f2.len());

    Ok(())

}

fn lcs_para_lineas(lineas_f1: &Vec<String>, lineas_f2: &Vec<String>) -> Vec<Vec<u32>> {

    let cantidad_filas_para_grilla = lineas_f1.len() + 1;
    let cantidad_columnas_para_grilla = lineas_f2.len() + 1;

    let mut grilla: Vec<Vec<u32>> = vec![vec![0; cantidad_columnas_para_grilla]; cantidad_filas_para_grilla];
...
///    i=len(X) y j=len(Y)
fn print_diff(grilla: &Vec<Vec<u32>>,lineas_f1: &Vec<String>, lineas_f2: &Vec<String>, i: usize, j: usize) {
